class Search extends Component {
  ...
  handleSearch(event) {
    ...
    if (key === 'Enter') {
      if (prefix.indexOf('!') === 0) {
        // Проверка на случай, если ввели префикс, которого нет в конфиге
        const engineData = this.engines[prefix.substr(1)];
        if (engineData) {
            engine = engineData[0];
            args = args.slice(1);
        }
      ...
      const finalUrl = engine + encodeURI(args.join(' '));

      // --- ВОТ ЭТА МАГИЯ ВМЕСТО window.location ---
      this.dispatchEvent(new CustomEvent('open-link', {
        detail: { url: finalUrl },
        bubbles: true,   // Позволяет событию подняться вверх по DOM
        composed: true   // Позволяет событию выйти за пределы Shadow DOM
      }));

      this.deactivate(); // Закрываем панель поиска после нажатия Enter
      target.value = ''; // Очищаем инпут
    }
